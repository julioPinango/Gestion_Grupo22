...
create-groups:
	docker exec -it billbudy_db psql -U billbudyUser -d billbudy -c "CREATE TABLE IF NOT EXISTS groups (id SERIAL PRIMARY KEY, name VARCHAR(100), description VARCHAR(400), admin VARCHAR(100), objetive INT, saving INT, FOREIGN KEY (admin) REFERENCES users(username));"
